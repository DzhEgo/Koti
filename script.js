...
class Kot {
    constructor(kit){
        this.name = kit.name
        this.img_link = kit.img_link
        this.age = kit.age
        this.rate = kit.rate
        this.favourite = kit.favourite
...
for(let e of data){
    con.innerHTML += 
    `<div class='kartochka' style = 'margin: 10px; border-radius: 7px; background-color: #ffe4e1;'>
    <h1>${e.name}</h1>
    <h1>ID: ${e.id}</h1>
    <img src='${e.img_link}'height=400 width=400 style = 'border-radius: 10px'></img>
    <h2>Рейтинг - ${e.rate}<h2>
    <h3>Возраст - ${e.age}</h3>
    <p>${e.description}</p>
    <span>Любимая? - ${e.favourite}</span>
</div>`
}  